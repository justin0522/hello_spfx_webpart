...
import { IKanbanBoardProps } from './IBoardProps';
...
export default class KanbanBoard extends React.Component<IKanbanBoardProps, ICustomKanbanProps> {
    ...
    public componentDidUpdate(prevProps, prevState) {
        if (prevState.listTitle !== this.state.listTitle) {
            ...
            this._getData(this.props.listTitle).then(() => console.log('end'));
        }
    }
    ...
    _getData = async (listTitle): Promise<void> => {
        if (listTitle && listTitle.length > 0) {
            let myWeb = Web('https://m365x367613.sharepoint.com/sites/myteam');
            let status: any = await myWeb.lists.getByTitle(listTitle).fields.getByInternalNameOrTitle("Status").get();

            const cols = status.Choices.map((val, index) => {
                return { text: val, dataField: val };
            });

            let res = await myWeb.lists.getByTitle(listTitle).items.getAll();
            ...
            let users = await myWeb.siteUsers.get();
